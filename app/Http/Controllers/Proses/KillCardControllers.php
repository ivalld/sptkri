...
use App\Models\Master\DokumenKapal;
use App\Models\Master\KillCard;
use Validator;
use Response;
use View;
use DB;
...
class KillCardControllers extends Controller
{
    ...
    {
        $killcard      = KillCard::OrderBy('no_lambung', 'desc')->get();
        $kelompok       = DokumenKapal::OrderBy('id', 'desc')->get();
        return view('master.kelompok.kelompok', compact('kelompok', 'kapal'));
    }
    ...
    /*** Add Dokumen Kapal ***/
    public function createDokumenKapal()
    {
    ...
    {
        $kelompok = DokumenKapal::OrderBy('id', 'desc')->where('nama_pokok', 'like', "%{$request->pencarian}%")->get();
        return view('master.kelompok.data_list_kelompok', compact('kelompok'));
    }

    /*** Edit Kompnen Pokok ***/
    public function editDokumenKapal($id)
    {

        $data_kelompok = DokumenKapal::where('id', '=', "{$id}")->get()->first();
    ...
    /*** Update Dokumen Kapal ***/
    public function updateDokumenKapal(Request $request, $id)
    {
        $validasi = Validator::make($request->all(), [
    ...
    public function deleteDokumenKapal(Request $request)
    {
        DokumenKapal::where('id', '=', "{$request->id_hapus}")->delete();
