...
use App\Models\Master\PemeliharaanModel;
...
class KartuPemeliharaanControllers extends Controller
{
    ...
    {
        $kapal               = KapalModel::OrderBy('id_kapal', 'desc')->get();
        ...
        $kartu_pemeliharaan  = PemeliharaanModel::Orderby('id', 'desc')->get();
        $paginate            = 1;
        ...
        $pemeliharaan   = DB::table('kartu_pemeliharaan as kp')
            ...
            ->leftjoin('kapal as kap', 'kp.id_kapal', '=', 'kap.id_kapal')
            // ->leftjoin('kode_pemeliharaan as kdpemeliharaan', 'kp.kode_pemeliharaan', '=', 'kdpemeliharaan.id')
            ->select(
                'kp.tanggal',
                'kp.kode_pemeliharaan',
                'kp.id_komponen_pokok',
                'kp.id_komponen_sub_pokok',
                'kp.id_sistem',
                'kp.id_sub_sistem',
                'kp.id_pelaksana',
                'kp.id_lokasi',
                'kp.uraian_pemeliharaan',
                'kp.tindakan_pengamanan',
                'kp.prosedur',
                'komp.kode_pokok',
                'komp.nama_pokok',
                'ksp.kode_sub_pokok',
                'ksp.nama_sub_pokok',
                'ks.kode_komponen_sistem',
                'ks.nama_komponen_sistem',
                'kss.kode_komponen_sub_sistem',
        ...
        return view(
            'proses.kartu_pemeliharaan.kartu_pemeliharaan',
            compact(
                'paginate',
                'pemeliharaan',
                'kelompok',
    ...
    public function createKartuPemeliharaan()
    {
        ...
        return view('proses.kartu_pemeliharaan.create_kartu_pemeliharaan', compact(
            'kelompok',
            'subkelompok',
            'sistem',
            'subsistem',
            ...
            if (isset($data_pemeliharaan)) {
                ...
                if ($data_skedul > 0) {
                    ...
                    if ($jml_hari > 365) {
                        ...
                        if (isset($cek_status)) {
                            return redirect()->back()->with('gagal', 'Skedul perencanaan pemeliharaan sudah ada dalam database');
                            exit();
                        }
    ...
    public function printKartuPemeliharaan($id)
    {
        $pemeliharaan = DB::table('kartu_pemeliharaan as kp')
            ...
            ->select(
                ...
                'kp.tgl_mulai',
                'kp.alat_kerja',
                'per.keterangan as ket_periode',
                'kap.nama_kapal as nama_kapal',
                'kap.no_lambung as no_lambung',
            )
            ->where('kp.id', '=', "{$id}")
            ->OrderBy('kp.id', 'desc')->get()->first();
        if (isset($pemeliharaan)) {
